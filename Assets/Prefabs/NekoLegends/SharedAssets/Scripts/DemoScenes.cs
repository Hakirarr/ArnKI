...
using System; // Required for Environment.Exit
...
namespace NekoLegends
{
    public class DemoScenes : MonoBehaviour
    {
        ...
        private void EscapeAction_performed(InputAction.CallbackContext context)
        {
            QuitApplication();
        }
#endif
        ...
        {
            if (CameraDOFDatas != null && CameraDOFDatas.Count > 0)
            {
                float currentFocusDistance = _depthOfField.focusDistance.value;
                float currentAperture = _depthOfField.aperture.value;
        ...
        {
            // Old Input System: Listen for the Escape key
