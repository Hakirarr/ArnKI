...
namespace NekoLegends
{
    public class MiniGameGUI : MonoBehaviour
    {
        ...
        public int UpdateRank()
        {
            ...
            {
                Debug.Log("checking rank: " + score + " " + scoreThresholds[i]);
                if (score >= scoreThresholds[i])
                {
                    // Set the rank title based on the selected rank
                    rankTitle.text = rankTitles[i];

                    // Calculate the rank number by inverting the index
                    rankNumber = rankTitles.Length - i;

                    // Update the rank number text
